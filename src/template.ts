...
import {
	FileDescriptorProto, DescriptorProto, EnumDescriptorProto, ServiceDescriptorProto, MethodDescriptorProto,
...
const typeCast = (field: FieldDescriptorProto, mapEntriesMap: {[key: string]: DescriptorProto}, fileName: string): string => {
	...
	switch (type) {
		...
		case types.TYPE_MESSAGE: {
			if (mapEntriesMap[typeName]) {
				typeStr = `{[key: string]: ${typeCast(mapEntriesMap[typeName].getFieldList()[1], {}, fileName)}}`
			}else {
				typeStr = context.getTypeName(typeName, fileName);
			}
			break;
...
const renderAllMessages = (messages: DescriptorProto[] = [], packageName: string, fileName: string, parentTypeName = "") => {
	return messages
		.map(message => {
			...
			const nestedTypes = message.getNestedTypeList().filter((type)=>(type.getOptions() === undefined || !type.getOptions().getMapEntry()));
			let retStr = "";
			...
			const mapEntriesMap = message.getNestedTypeList().filter((type)=>(type.getOptions() && type.getOptions().getMapEntry())).reduce<{[key: string]: DescriptorProto}>((pValue, cValue)=>{
				return {
					...pValue,
...
const renderAllEnums = (enums: EnumDescriptorProto[] = [], parentTypeName= "") => {
...
${oneEnum.getValueList().map(value => `	${value.getName()} = "${value.getName()}",`).join(lineSplitter)}
}
...
const renderMethods = (methods: MethodDescriptorProto[], packageName: string, serviceName: string, fileName: string) => {
	const context = getImportedTypesContext();
	return methods
		.map(method => {
...
const renderService = (service: ServiceDescriptorProto, packageName: string, fileName: string) => {
	return `
${renderMethods(service.getMethodList(), packageName, service.getName(), fileName)}

export default {
${service.getMethodList().map(method => `	${method.getName()},`).join(lineSplitter)}
};
`;
};

function renderImportSection(hasService: boolean, webapiPath: string, fileName: string) {
...
const template = (
	...
	const webapiPath =
			...
			? apiPath
			: join(strRepeat("../", packageName.split(/\./g).length), apiPath);
